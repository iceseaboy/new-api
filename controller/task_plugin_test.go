...
func klingFactoryVersion(t *testing.T) string {
	t.Helper()
	factorySource, err := plugins.Source("vidu")
	require.NoError(t, err)
...
func TestDisableFactoryPluginPersistsOptionAndHidesFromBindOptions(t *testing.T) {
	...
	assert.Equal(t, `["vidu"]`, stored.Value)
...
func TestDisableFactoryPluginRespectsInUseGuard(t *testing.T) {
	...
	const key = "vidu"
	baseURL := "https://example.com"
	channelSetting := `{"task_plugin_key":"vidu"}`
...
func TestDisableFactoryOverrideRowSuppressesBothLayers(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { jsplugin.DefaultRegistry.Unregister("vidu") })
	plugin := model.TaskPlugin{
		Key: "vidu", APIVersion: loaded.Meta.APIVersion, Version: loaded.Meta.Version,
	...
	recorder := postTaskPluginStatus(t, "vidu", "", `{"enabled":false}`)
	assert.Contains(t, recorder.Body.String(), `"success":true`)
	assert.Equal(t, []string{"vidu"}, setting.GetTaskPluginDisabledFactoryKeys())
	row, err := model.GetTaskPluginVersion("vidu", "")
	require.NoError(t, err)
	assert.False(t, row.Enabled)
	item := listTaskPluginItem(t, "vidu")
	assert.Equal(t, "override_over_factory", item.Source)
	assert.False(t, item.Enabled)
	assert.Equal(t, "disabled", item.RuntimeStatus)
	assert.False(t, taskPluginOptionsHasKey(t, "vidu"))
	_, ok := jsplugin.DefaultRegistry.Get("vidu")
	assert.False(t, ok, "factory built-in must not keep serving after the plugin is switched off")

	recorder = postTaskPluginStatus(t, "vidu", "", `{"enabled":true}`)
	assert.Contains(t, recorder.Body.String(), `"success":true`)
	assert.Empty(t, setting.GetTaskPluginDisabledFactoryKeys())
	row, err = model.GetTaskPluginVersion("vidu", "")
	require.NoError(t, err)
	assert.True(t, row.Enabled)
	got, ok := jsplugin.DefaultRegistry.Get("vidu")
	require.True(t, ok)
	assert.Equal(t, loaded.Meta.Version, got.Meta.Version)
	assert.Equal(t, "registered", listTaskPluginItem(t, "vidu").RuntimeStatus)
...
func TestListTaskPluginsIncludesFactoryWithoutDatabaseRows(t *testing.T) {
	...
	for i := range response.Data {
		if response.Data[i].Meta.Key == "vidu" {
...
func TestMasterSwitchEmptiesOptionsAndKeepsList(t *testing.T) {
	...
	item := listTaskPluginItem(t, "vidu")
	assert.Equal(t, "factory", item.Source)
	assert.Equal(t, "vidu", item.Meta.Key)
}
...
func TestDeleteActiveOverrideFallsBackToFactoryAndDeletesRecord(t *testing.T) {
	...
	require.NotEqual(t, factorySource, overrideSource, "factory version marker must be found in kling source")
	loaded, err := jsplugin.DefaultRegistry.Register(overrideSource, jsplugin.Options{Key: "vidu", Version: "test-override"})
	require.NoError(t, err)
	t.Cleanup(func() { jsplugin.DefaultRegistry.Unregister("vidu") })
	plugin := model.TaskPlugin{
		Key: "vidu", APIVersion: loaded.Meta.APIVersion, Version: loaded.Meta.Version,
	...
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Params = gin.Params{{Key: "key", Value: "vidu"}, {Key: "version", Value: loaded.Meta.Version}}
	context.Request = httptest.NewRequest(http.MethodDelete, "/api/plugin/task/kling/versions/"+loaded.Meta.Version, nil)

	DeleteTaskPluginVersion(context)

	assert.Contains(t, recorder.Body.String(), `"success":true`)
	versions, err := model.ListTaskPluginVersions("vidu")
	require.NoError(t, err)
	assert.Empty(t, versions)
	runtimePlugin, ok := jsplugin.DefaultRegistry.Get("vidu")
	require.True(t, ok)
...
func TestUploadTaskPluginPreflightConflict(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantError: "channelType 52 conflicts",
		},
		{
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			source := taskPluginControllerChannelSource(testCase.key, "1.0.0", 52)
			...
			assert.Contains(t, recorder.Body.String(), "vidu")
			var count int64
...
func TestDeletePureFactoryPluginIsRejected(t *testing.T) {
	for _, query := range []string{"", "?force=true"} {
		t.Run(query, func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			context, _ := gin.CreateTestContext(recorder)
			context.Params = gin.Params{{Key: "key", Value: "vidu"}, {Key: "version", Value: "1.0.0"}}
