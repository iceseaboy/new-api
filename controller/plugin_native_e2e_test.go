...
func TestKlingNativeRouteSubmitPollSettleAndQuery(t *testing.T) {
	...
	// 方式临时注册内嵌源码，仅用于验证插件原生路由的通用机制。
	klingSource, sourceErr := plugins.Source("kling")
	require.NoError(t, sourceErr)
	_, registerErr := pluginruntime.DefaultRegistry.Register(klingSource, pluginruntime.Options{})
	require.NoError(t, registerErr)
	t.Cleanup(func() { _ = pluginruntime.DefaultRegistry.Unregister("kling") })
