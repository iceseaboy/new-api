...
func TestGetTaskAdaptorMapsMigratedPlatformsToFactoryPlugins(t *testing.T) {
	// 本仓库对 ali/kling/jimeng/doubao/submodel 保留内置 Go 适配器
	// （见 builtinTaskAdaptor），其余平台走上游工厂插件。
	pluginPlatforms := []constant.TaskPlatform{
	...
	for _, platform := range pluginPlatforms {
	...
	builtinPlatforms := []constant.TaskPlatform{
		...
		constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeSubmodel)),
	}
	for _, platform := range builtinPlatforms {
		adaptor := GetTaskAdaptor(platform)
		require.NotNil(t, adaptor, "platform %s should resolve", platform)
		_, isJS := adaptor.(*jspluginadaptor.TaskAdaptor)
		assert.False(t, isJS, "platform %s should use the built-in Go adaptor", platform)
	}
}
