...
func TestProductionPluginNativeQueryTraversesInnerRouter(t *testing.T) {
	// kling 工厂插件在运行时被跳过（由内置 Go 适配器承载），此处按 override
	// 方式临时注册内嵌源码，仅用于验证插件路由机制本身。
	klingSource, klingSourceErr := builtinplugins.Source("kling")
	require.NoError(t, klingSourceErr)
	_, klingRegisterErr := jsplugin.DefaultRegistry.Register(klingSource, jsplugin.Options{})
	require.NoError(t, klingRegisterErr)
	t.Cleanup(func() { _ = jsplugin.DefaultRegistry.Unregister("kling") })
