...
func TestGetOpenAIVideoRouteRendersJimengTask(t *testing.T) {
	// jimeng 工厂插件在运行时被跳过（由内置 Go 适配器承载），此处按 override
	// 方式临时注册内嵌源码，仅用于验证插件路由机制本身。
	jimengSource, jimengSourceErr := builtinplugins.Source("jimeng")
	require.NoError(t, jimengSourceErr)
	_, jimengRegisterErr := jsplugin.DefaultRegistry.Register(jimengSource, jsplugin.Options{})
	require.NoError(t, jimengRegisterErr)
	t.Cleanup(func() { _ = jsplugin.DefaultRegistry.Unregister("jimeng") })
