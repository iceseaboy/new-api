...
func LogTaskConsumption(c *gin.Context, info *relaycommon.RelayInfo, task *model.Task) {
	...
	if info.PriceData.GroupRatioInfo.HasUserModelRatio {
		other.SetPublic("user_model_ratio", info.PriceData.GroupRatioInfo.UserModelRatio)
	}
...
func taskBillingOther(task *model.Task) *model.LogOther {
	...
	if bc := task.PrivateData.BillingContext; bc != nil {
		...
		if bc.Resolution != "" {
			other.SetPublic("resolution", bc.Resolution)
		}
		if bc.TotalTokens > 0 {
			other.SetPublic("total_tokens", bc.TotalTokens)
		}
	}
...
func RefundTaskQuota(ctx context.Context, task *model.Task, reason string) bool {
	...
	model.RecordTaskBillingLog(model.RecordTaskBillingLogParams{
		...
		UpstreamRequestId: task.Properties.UpstreamRequestId,
		Other:             other,
	})
...
func RecalculateTaskQuotaByTokens(ctx context.Context, task *model.Task, totalTokens int) bool {
	...
	if userModelRatio, hasUserModelRatio := ratio_setting.GetUserModelRatio(task.UserId, modelName); hasUserModelRatio {
	...
	if actualQuota == 0 {
		// 乘积不足 1 quota 时保持预扣不动（沿用旧语义）；<0 的放宽仅服务 tiered 结算的"合法算得 0 应全退"。
		logger.LogInfo(ctx, fmt.Sprintf("任务 %s token重算额度为 0，保留预扣额度", task.TaskID))
		return true
	}
