...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	if err = common.Unmarshal(responseBody, &responseItems); err == nil && responseItems.IsSuccess() {
		...
		if taskResult.Status == string(model.TaskStatusSuccess) && len(t.Data) > 0 {
			if inner, innerErr := adaptor.ParseTaskResult(task, resp, t.Data); innerErr == nil && inner != nil {
				if taskResult.Url == "" {
...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) bool {
	...
	// 失败任务一律交由调用方全额退款：我方 Go 适配器的 AdjustBillingOnComplete
	// 直接从 task.Data 反序列化 usage 计算额度，失败响应携带的 usage 不应触发补扣。
	if task.Status == model.TaskStatusFailure {
		return false
	}
	// 优先让 adaptor 决定最终额度（先于按次跳过：如 ali happyhorse video-edit
	// 提交时时长未知，即使按次计费也需按上游 usage 实际时长差额结算）。
