...
func TestSettle_PerCallBilling_AdaptorAdjustTakesPriority(t *testing.T) {
	truncate(t)
	ctx := context.Background()
	...
	// 按次计费但 adaptor 返回正数时仍差额结算（如 video-edit 提交时时长未知），
	// 按次跳过仅对 adaptor 返回 0 的情况生效
...
func TestSettle_FailedTaskSkipsAdaptorAdjustmentForFullRefund(t *testing.T) {
	truncate(t)
	ctx := context.Background()
	...
	seedUser(t, userID, initQuota)
	seedToken(t, tokenID, userID, "sk-failed-adj", 8000)
	seedChannel(t, channelID)

	task := makeTask(userID, channelID, preConsumed, tokenID, BillingSourceWallet, 0)
	task.Status = model.TaskStatusFailure

	// 失败响应可能携带 usage：适配器即使能算出正额度也不得接管结算，
	// 必须返回 false 让调用方全额退款
	adaptor := &mockAdaptor{adjustReturn: 3000}
	taskResult := &relaycommon.TaskInfo{Status: string(model.TaskStatusFailure)}
